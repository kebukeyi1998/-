<template>
	<gracePage>
		...
		<view class="grace-header-body" slot="gHeader">
			<text class="grace-header-icons grace-icons icon-arrow-left" @tap.stop="goback"></text>
			<view class="grace-header-content grace-flex-center">
				<text class="grace-header-text grace-ellipsis">消息中心</text>
			</view>
		</view>
		<!-- 页面主体 -->
		<view slot="gBody">
			...
			<view class="grace-body grace-bg-white" v-for="(msgBody,index) in msgs" :key="index">
				...
				<view class="grace-list">
					<view class="grace-list-items" v-for="(msg, indexMsg) in msgBody.msgsList" :key="indexMsg">
						...
						<view 
						class="grace-list-body" :data-msgindex="index + ',' + indexMsg" @tap="gotoinfo" 
						...
						<view class="grace-fun-btn" :style="{width:funBtnsWidthShow+'px'}" v-if="moveindex == index + ',' + indexMsg" @tap.stop="">
...
.grace-header-icons{font-size:18px;}
